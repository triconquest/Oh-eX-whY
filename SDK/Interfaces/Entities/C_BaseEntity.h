...
public:
	...
	virtual bool					ShouldDrawWaterImpacts(void) = 0;                                            // int* down here instead of int
	virtual bool					HandleShotImpactingWater(const FireBulletsInfo_t& info, const Vector& vecEnd, int* pTraceFilter, Vector* pVecTracerDest) = 0;
	virtual int*			GetBeamTraceFilter(void) = 0; // int*
	virtual void					DispatchTraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, int* ptr, CDmgAccumulator* pAccumulator = NULL) = 0;
	virtual void					TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, int* ptr, CDmgAccumulator* pAccumulator = NULL) = 0;
	virtual void					DoImpactEffect(int& tr, int nDamageType) = 0;
	virtual void					MakeTracer(const Vector& vecTracerSrc, const int& tr, int iTracerType) = 0;
	...
	virtual bool					TestCollision(const int& ray, unsigned int fContentsMask, int& tr) = 0;
	virtual bool					TestHitboxes(const int& ray, unsigned int fContentsMask, int& tr) = 0;
	...
	virtual void					AddDecal(const Vector& rayStart, const Vector& rayEnd, const Vector& decalCenter, int hitbox, int decalIndex, bool doTrace, int& tr, int maxLODToDecal) = 0;
	virtual void					AddColoredDecal(const Vector& rayStart, const Vector& rayEnd, const Vector& decalCenter, int hitbox, int decalIndex, bool doTrace, int& tr, Color cColor, int maxLODToDecal) = 0;
	...
	virtual void					DecalTrace(int* pTrace, char const* decalName) = 0;
	virtual void					ImpactTrace(int* pTrace, int iDamageType, const char* pCustomImpactName) = 0;
...
public:
	NETVAR(m_vecMins, Vector3, "CBaseEntity", "m_vecMins");
	NETVAR(m_vecMaxs, Vector3, "CBaseEntity", "m_vecMaxs");
	NETVAR(m_vecOrigin, Vector3, "CBaseEntity", "m_vecOrigin");
...
public:
	...
	{
		static int nOffset = NetVar.Get("CCSPlayerResource", "m_iHealth");
