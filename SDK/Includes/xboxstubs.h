...
// 	unsigned char        abEnet[6];                      // Ethernet MAC address
// 	unsigned char        abOnline[20];                   // Online identification
// } XNADDR;
...
typedef struct {
	unsigned char        ab[8];                          // xbox to xbox key identifier
} XNKID;

typedef struct {
	unsigned char        ab[16];                         // xbox to xbox key exchange key
} XNKEY;

typedef struct _XSESSION_INFO
{
	XNKID sessionID;                // 8 unsigned chars
	XNADDR hostAddress;             // 36 unsigned chars
	XNKEY keyExchangeKey;           // 16 unsigned chars
...
__forceinline void			XBX_ProcessEvents() {}
__forceinline unsigned int	XBX_GetSystemTime() { return 0; }
__forceinline int				XBX_GetPrimaryUserId() { return 0; }
__forceinline void			XBX_SetPrimaryUserId(DWORD idx) {}
__forceinline int				XBX_GetStorageDeviceId() { return 0; }
__forceinline void			XBX_SetStorageDeviceId(DWORD idx) {}
__forceinline const char* XBX_GetLanguageString() { return ""; }
__forceinline bool			XBX_IsLocalized() { return false; }
